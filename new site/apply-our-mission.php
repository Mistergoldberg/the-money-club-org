...
$allowed_returns = ['support-our-mission.html'];
$return_to = tmc_resolve_return_target($_POST['return-to'] ?? 'support-our-mission.html', $allowed_returns, 'support-our-mission.html');
$error_return = tmc_resolve_return_target($_POST['return-error'] ?? $return_to, $allowed_returns, 'support-our-mission.html');

try {
    tmc_issue_csrf_cookie();
} catch (RuntimeException $e) {
    tmc_log_form_security_event('apply-our-mission', 'csrf_cookie_failed');
...
if (tmc_honeypot_triggered()) {
    tmc_log_form_security_event('apply-our-mission', 'honeypot_tripped', ['return_to' => $return_to]);
    tmc_redirect_with_status($return_to, 'sent');
}

$rate_limit = tmc_rate_limit_check('apply-our-mission', 6, 600);
if (!$rate_limit['allowed']) {
    tmc_log_form_security_event('apply-our-mission', 'rate_limited', ['retry_after' => (string)$rate_limit['retry_after']]);
...
if (!tmc_verify_csrf_token(true, $csrf_reason)) {
    tmc_log_form_security_event('apply-our-mission', 'csrf_failed', ['reason' => $csrf_reason]);
...
if (!smtp_send_mail($to, $subject, $message, $from, $email)) {
    tmc_log_form_security_event('apply-our-mission', 'internal_email_failed');
